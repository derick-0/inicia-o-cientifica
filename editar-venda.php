...
<form action="?page=salvar-venda" method="POST">
    ...
    <div class="mb-3">
        <label>remedio (produto)</label>
        <select name="produto_id_produto" class="form-control" required>
            <?php
            $sql_modelos = "SELECT id_produto, nome_produto, receita_produto FROM produto";
            $res_modelos = $conn->query($sql_modelos);
            while ($row_modelo = $res_modelos->fetch_object()) {
                $selected = ($row_modelo->id_produto == $row_venda->produto_id_produto) ? 'selected' : '';
                print "<option value='" . $row_modelo->id_produto . "' " . $selected . ">" . $row_modelo->nome_produto . " (" . $row_modelo->receita_produto . ")</option>";
            }
            ?>
        </select>
    </div>
